...
| superfrete | Minhas Postagens - Portal do Cliente | cliente externo | `/superfrete` | etiquetas/SuperFrete | baseline criada |
| superfrete-admin | SuperFrete Admin | interno | `/superfrete-admin` | etiquetas + integrações | baseline criada |
| balcao | Balcão / Etiquetas | interno | `/balcao` | etiquetas + CEP + caixa | baseline criada |
...
| agf | Acesso AGF | interno / legado em transição | `/agf` | autenticação + atende | baseline criada |
...
- `apps-script/base-cliente-etiquetas`: documentado como serviço de apoio a etiquetas.
- `apps-script/logistica`: documentado na família Reverso.
- `apps-script/caixa`: documentado em Balcão/Caixa quando o vínculo estiver confirmado.
...
7. Atualizar `docs/MAPA_MODULOS.md` à medida que cada pendência for validada.
