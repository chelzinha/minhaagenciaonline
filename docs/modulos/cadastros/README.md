...
Módulo independente em `frontend/cadastros/` e `cloudflare/cadastros-api/`, com D1 `agf-cadastros`. O D1 foi criado e o esquema inicial aplicado em 2026-09-24. O Worker e o cron foram ativados após autorização da gestora; o frontend está no preview do PR e ainda não foi integrado à `main`. A primeira varredura foi concluída: 93.624 postagens importadas, iguais ao total da view canônica na conferência de 24/09/2026; 301 clientes diretos criados. A revisão de identidade na tela depende de sessão administrativa. A integração de leitura pelo CRM existente será uma etapa separada. Nada é gravado no D1 do Atende, no RAW, no Apps Script do CRM ou no `agf-core`.
...
| --- | --- | --- |
| `BALCÃO` | `NOME REMETENTE` | Alias `SENDER` confirmado tem prioridade; na falta dele, nome normalizado idêntico a um `CLIENTE PORTAL` direto usa esse mesmo cliente. O restante fica pendente. |
...
`key()` em `src/identity.js` só aplica `trim`, maiúsculas, remoção de acentos e compactação de espaços. Não elimina pontuação, sufixos societários ou palavras. A limpeza semântica é um alias `kind + normalized_name → customer_id`, criado pela tela e registrado no `audit_log`. Um alias `SENDER` só vale para os três portais compartilhados; um `PORTAL` só vale para os demais como chave direta. A coincidência exata entre o remetente compartilhado e um alias `PORTAL` direto também resolve a identidade; não cria alias novo e nunca vence um alias `SENDER` confirmado. Colisão/transferência de alias exige leitura da associação anterior e confirmação explícita. Nenhum contrato ou cartão produz associação automática, pois esses identificadores podem ocorrer em mais de um cliente.
...
O cron ativo roda a cada minuto e lê até vinte páginas de 200 operações na primeira passagem. Nas passagens seguintes, só processa nos minutos múltiplos de dez. O botão administrativo lê uma página. A tela mostra separadamente postagens importadas, clientes cadastrados, nomes distintos de remetentes ainda pendentes e postagens pendentes. A lista de clientes tem paginação com total de resultados. Repetições são idempotentes. Cada passagem completa atualiza registros existentes, detecta mudança de portal/remetente/LOCAL na origem, preserva override manual se a chave de origem não mudou e remove projeções que deixaram de ser canônicas. Se o portal/remetente mudar, o override anterior deixa de valer e a identidade é resolvida novamente. A primeira varredura integral demanda vários ciclos; o painel mostra a contagem de passagens e o cursor. O cursor da origem é `r.id`, não SRO nem contrato.

Diagnóstico do agendamento: a primeira tentativa atualiza `sync_state.updated_at`; falhas técnicas são registradas como `SYNC_ERROR` no `audit_log`, sem payload de postagem ou token. A consulta de origem do Worker foi executada diretamente no D1 do Atende e retornou 200 linhas. O cron iniciou após a propagação da agenda. Houve um `SYNC_ERROR` às 02:02 UTC causado por concorrência esperada entre ciclos; a primeira varredura concluiu depois. O Worker passou a ignorar apenas esse conflito de exclusão mútua, preservando o registro de erros reais. Na conferência, 77.009 postagens foram ligadas por portal direto e 16.615 ficaram pendentes (16.148 de portais compartilhados e 467 sem portal). Oito postagens distribuídas pela origem tiveram remetente, portal, LOCAL, contrato, cartão, data e valor iguais aos do Atende. Falta testar a tela autenticada e os fluxos administrativos antes de integrar à `main`.
...
2. O Worker e o cron foram ativados na conta correta após autorização. `/health` respondeu 200, `/api/status` sem sessão respondeu 401 e o preflight CORS do preview foi permitido. A primeira passagem e os totais foram confirmados, com amostra de LOCAL e demais campos. Falta conferir o acesso administrativo autenticado. O arquivo `wrangler.jsonc` é a fonte da configuração de publicação seguinte.
3. Conferir autorização administrativa e um fluxo de associação na tela; a comparação de oito amostras da origem e o total da primeira passagem já foram realizados. Os aliases manuais confirmados têm prioridade. A planilha histórica `CLIENTES_ALIAS` possuía 2.484 linhas marcadas `AUTO`, cada uma com ID distinto na inspeção de 24/09; ela não é um dicionário de equivalências comerciais revisadas e não deve ser importada como se fosse. O arquivo `CADASTRO_MESTRE_CLIENTES.xlsx` tem 9.181 grafias legadas, 2.140 propostas em lote seguro e 13 decisões manuais validadas. Sua aba `01_CLIENTES_MASTER` está vazia e os aliases estão em `PENDENTE_CLIENTE_ID`, portanto IDs propostos não são IDs do D1. No cruzamento de nomes, cerca de 3.600 pendências atuais aparecem na prévia, mas somente 330 nomes têm status `PRONTO_PREVIA`; 31 desses apontam por nome a um portal direto já existente. As regras de Centro/Local e os grupos analíticos não são identidade. A IA pode sugerir equivalências para aprovação em lote, mas não altera identidade por semelhança sozinha.
