...
const EXPEDICAO_STANDALONE = true;
const $ = (id) => document.getElementById(id);
const state = {
  tab: 'expedicao', unidades: [], lotes: [], reversas: [], coletas: [], divergencias: [], params: [], coletadores: [],
...
function renderUnidadesTable(items, withActions=true) {
  return table([
    ...
    {key:'status_ponto',label:'Etiquetas',render:r=>r.status_ponto?.etiquetas_disponiveis||0}, {key:'coletador_padrao_id',label:'Coletador responsável',render:r=>safe(collectorLabel(r.coletador_padrao_id))}, {key:'prazo_coleta_dias_uteis',label:'Prazo'}
      ...
      ? `<button class="btn btn-ghost btn-sm" data-edit-unit="${u.unidade_id}">Editar</button><button class="btn btn-ghost btn-sm" data-open-coleta="${u.unidade_id}">Abrir coleta</button>`
      : `${iconButton('visibility','Gerenciar unidade',`data-manage-unit="${u.unidade_id}"`)}${iconButton('local_shipping','Abrir coleta',`data-open-coleta="${u.unidade_id}"`)}`
...
async function renderTab(tab=state.tab){if(EXPEDICAO_STANDALONE)tab='expedicao';state.tab=tab;document.querySelectorAll('.admin-tab').forEach(b=>b.classList.toggle('is-active',b.dataset.tab===tab));await renderers[tab]();}
function boot(){const user=auth.getCachedUser()||auth.getLocalSession()?.user||{};$('internalUserName').textContent=user.displayName||user.username||'Usuário';if($('todayReference'))$('todayReference').textContent=todayLabel();state.tab='expedicao';$('btnRefresh').addEventListener('click',()=>{invalidateDataCache(); renderExpedicao();});$('logoutButton').addEventListener('click',async()=>{await auth.logout();auth.redirectToLogin('login')});document.getElementById('adminTabs')?.addEventListener('click',e=>{const btn=e.target.closest('[data-tab]');if(!btn)return;renderTab(btn.dataset.tab)});renderTab('expedicao');}
