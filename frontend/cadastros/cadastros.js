  ...
  function desenharFicha(r) {
    ...
    el.innerHTML = `
      ...
      <div class="f-sec"><h3><span class="material-symbols-rounded">description</span>Contratos observados <em>${r.contratos.length}</em></h3>
        ${r.contratos.length ? `<table class="f-tbl"><thead><tr><th>Contrato</th><th>Cartão</th><th class="r">Post.</th><th class="r">Última</th></tr></thead><tbody>
          ${r.contratos.map((k) => `<tr><td>${esc(k.contrato || '-')}</td><td>${esc(k.cartao || '-')}</td><td class="n">${num(k.postagens)}</td><td class="n">${dataBr(k.ultima)}</td></tr>`).join('')}</tbody></table>`
