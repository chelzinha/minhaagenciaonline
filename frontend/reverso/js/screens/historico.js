...
export async function mount(){
  ...
  try{ UI.showLoading('Buscando histórico...'); const data=await Api.getUserHistory(state.user.usuario_id); const items=data.items||[]; if(!items.length){ list.innerHTML=emptyState('package_2','Nenhuma devolução registrada ainda.'); return; }
