...
export async function mount(){
  ...
  async function load(){
    try{ UI.showLoading('Atualizando o painel...'); const data=await Api.getDashboard(); const stats=data.resumo||{}; grid.innerHTML=Object.entries(stats).map(([k,v])=>`<article class="stat-card"><div class="stat-label">${k.replaceAll('_',' ')}</div><div class="stat-value">${v}</div></article>`).join(''); }
