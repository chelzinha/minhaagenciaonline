...
(() => {
  ...
  function localEntry(p){
    return {id:p.entryId||uid(),batchId:p.batchId||'',batchIndex:p.batchIndex||1,date:p.date||todayIso(),createdAt:new Date().toISOString(),type:p.type,mode:p.mode,unitId:'PADRAO',operatorId:'local',operatorName:'Homologação local',clientId:p.clientId||'',clientName:p.clientName||'',clientSource:p.clientName?'INFORMADO':'SEM_CLIENTE',objectCount:p.objectCount||0,amountCents:p.amountCents,paymentId:p.paymentId,paymentName:state.library.payments.find(x=>x.id===p.paymentId)?.name||p.paymentId,categoryId:p.categoryId,description:p.description||'',pixStatus:p.pixStatus||'',pixTxid:p.pixTxid||'',pixProvider:p.pixProvider||'',status:'ATIVO',contaAzulStatus:'NAO_ENVIADO'};
  }
  ...
  function availableModes(){return state.type==='RECEITA'?[{id:'ATENDIMENTO',icon:'point_of_sale',label:'Atender'},{id:'AVULSO',icon:'add_card',label:'Avulso'},{id:'LOTE',icon:'playlist_add',label:'Lote'}]:[{id:'INDIVIDUAL',icon:'remove_circle',label:'Individual'},{id:'LOTE',icon:'playlist_add',label:'Lote'}];}
  function renderModes(){
    if(!availableModes().some(m=>m.id===state.mode))state.mode=state.type==='RECEITA'?'ATENDIMENTO':'INDIVIDUAL';
    $('modeSwitch').style.gridTemplateColumns=`repeat(${availableModes().length},1fr)`;
    $('modeSwitch').innerHTML=availableModes().map(m=>`<button class="mode-btn ${state.mode===m.id?'active':''}" data-mode="${m.id}"><span class="material-symbols-rounded">${m.icon}</span><span>${m.label}</span></button>`).join('');
  }
  function renderOptions(){
    $('categoryLabel').textContent=state.type==='RECEITA'?'Receita':'Despesa';
    $('categoryOptions').innerHTML=categories().map(x=>`<button class="option-btn ${state.selectedCategory===x.id?'active':''}" data-category="${x.id}" style="--option-color:${x.color|| (state.type==='RECEITA'?'#1677ff':'#ef4444')}"><span class="material-symbols-rounded">${x.icon||'category'}</span><span>${escapeHtml(x.name)}</span></button>`).join('');
    $('paymentOptions').innerHTML=payments().map(x=>`<button class="option-btn ${state.selectedPayment===x.id?'active':''}" data-payment="${x.id}" style="--option-color:${x.color||'#1677ff'}"><span class="material-symbols-rounded">${x.icon||'payments'}</span><span>${escapeHtml(x.name)}</span></button>`).join('');
  }
  ...
  async function saveSingle(){
    ...
    try{
      ...
      const result=await callApi('saveEntry',{payload:draft(state.amountCents,{pixStatus:selectedPayment().id==='PIX'?'CONFIRMADO':'',pixProvider:selectedPayment().id==='PIX'?'manual-single':''})});applySaveResult(result);status('launchStatus',state.type==='RECEITA'?'Receita registrada.':'Despesa registrada.','success');resetEntry();
