...
async function listAtende(url,env) {
  ...
  const select=`
    ...
    COALESCE(cp.cx_at,'') AS "_CX_AT_PORTAL",CASE WHEN pcl.local_codigo IS NOT NULL THEN 1 ELSE 0 END AS "_LOCAL_TRAVADO",
...
async function listFilters(url,env) {
  ...
  facetKeys.forEach((key,index)=>{
    ...
    const options=(results[index]?.results||[]).map(row=>{
      ...
      const label=key==='locais'?(localLabels.get(value)||value):value;
...
function mergeSelectedOptions(options,selected,key,localLabels) {
  const out=options.slice(),seen=new Set(out.map(x=>x.value));
  for(const value of unique((selected||[]).map(clean).filter(Boolean))) {
    if(!seen.has(value))out.push({value,label:key==='locais'?(localLabels?.get(value)||value):value,count:0});
  }
