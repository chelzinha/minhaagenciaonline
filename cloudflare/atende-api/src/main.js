  ...
  const [services, attendants, contracts, clients, historyRows] = await Promise.all([
    env.DB.prepare(`
      WITH raw_services AS (
        SELECT r.codigo_servico_norm AS codigo,
               MAX(r.nome_servico) AS nome_raw,
        ...
        GROUP BY r.codigo_servico_norm
      ), codes AS (
        SELECT codigo FROM raw_services
        UNION
        SELECT codigo_servico AS codigo FROM atende_servico_classificacao
      )
      SELECT codes.codigo,
             COALESCE(NULLIF(sc.nome_servico_referencia,''), rs.nome_raw, '') AS nome,
             COALESCE(rs.ocorrencias,0) AS ocorrencias,
             COALESCE(sc.tipo_servico,'') AS tipo_servico,
             COALESCE(sc.subgrupo,'') AS subgrupo,
             COALESCE(sc.tabela,'') AS tabela,
             COALESCE(sc.tipo_objeto,'') AS tipo_objeto
      FROM codes
      LEFT JOIN raw_services rs ON rs.codigo=codes.codigo
      LEFT JOIN atende_servico_classificacao sc ON sc.codigo_servico=codes.codigo
      ORDER BY COALESCE(NULLIF(sc.nome_servico_referencia,''), rs.nome_raw, codes.codigo) COLLATE NOCASE ASC,
               codes.codigo ASC
      LIMIT 3000
    `).all(),
    env.DB.prepare(`
    ...
    env.DB.prepare(`
      SELECT r.numero_contrato_norm AS numero,
             ...
             COALESCE(NULLIF(TRIM(co.tipo),''), CASE WHEN COUNT(*) BETWEEN 1 AND 3 THEN 'CONTRATO ECT' ELSE '' END) AS tipo,
             COALESCE(NULLIF(TRIM(co.nome),''), CASE WHEN COUNT(*) BETWEEN 1 AND 3 THEN 'CONTRATO ECT' ELSE '' END) AS nome
      FROM atende_postagens_raw r
      JOIN atende_raw_importacoes ri
        ON ri.import_key=r.import_key
       AND ri.concluido_em IS NOT NULL
      ...
      ORDER BY COALESCE(NULLIF(co.cliente,''),NULLIF(co.nome,''),r.numero_contrato_norm) COLLATE NOCASE ASC
      LIMIT 3000
    `).all(),
    env.DB.prepare(`
...
async function adminBulkServices(request, env) {
  ...
  if(source.length>2000)return json({ok:false,error:'max_2000_items'},413);
  ...
  for(const raw of source){
    ...
    const subgrupo=clean(raw?.subgrupo);
    const tabela=clean(raw?.tabela);
    const tipoObjeto=clean(raw?.tipoObjeto).toUpperCase();
    if(!codigo||seen.has(codigo))continue;
    if(tipoObjeto&&!['PRODUTO ECT','SEM REGISTRO'].includes(tipoObjeto))return json({ok:false,error:'invalid_tipo_objeto',codigo},400);
    seen.add(codigo);
    items.push({codigo,nomeServico,tipoServico,subgrupo,tabela,tipoObjeto});
  }
  if(!items.length)return json({ok:false,error:'valid_items_required'},400);

  const current=new Map(),rawNames=new Map();
  for(let i=0;i<items.length;i+=80){
    const chunk=items.slice(i,i+80),marks=chunk.map(()=>'?').join(',');
    const [classRows,rawRows]=await env.DB.batch([
      env.DB.prepare(`SELECT codigo_servico,nome_servico_referencia,tipo_servico,subgrupo,tabela,tipo_objeto FROM atende_servico_classificacao WHERE codigo_servico IN (${marks})`).bind(...chunk.map(x=>x.codigo)),
      env.DB.prepare(`
        ...
        WHERE r.codigo_servico_norm IN (${marks})
        GROUP BY r.codigo_servico_norm
      `).bind(...chunk.map(x=>x.codigo))
    ]);
    (classRows.results||[]).forEach(r=>current.set(clean(r.codigo_servico).toUpperCase(),r));
    (rawRows.results||[]).forEach(r=>rawNames.set(clean(r.codigo).toUpperCase(),clean(r.nome)));
  }
  ...
  for(const item of items){
    ...
    const rawName=rawNames.get(item.codigo)||'';
    const oldObj={
      nomeServico:clean(old?.nome_servico_referencia)||rawName,
      tipoServico:clean(old?.tipo_servico),
      subgrupo:clean(old?.subgrupo),
      tabela:clean(old?.tabela),
      tipoObjeto:clean(old?.tipo_objeto).toUpperCase()
    };
    const newObj={
      nomeServico:item.nomeServico||rawName,
      tipoServico:item.tipoServico,
      subgrupo:item.subgrupo,
      tabela:item.tabela,
      tipoObjeto:item.tipoObjeto
    };
    if(JSON.stringify(oldObj)===JSON.stringify(newObj)){unchanged++;continue;}

    const noMetadata=!newObj.tipoServico&&!newObj.subgrupo&&!newObj.tabela&&!newObj.tipoObjeto;
    const sameAsRaw=!newObj.nomeServico||newObj.nomeServico===rawName;
    if(noMetadata&&sameAsRaw){
      ...
      statements.push(env.DB.prepare(`
        INSERT INTO atende_servico_classificacao(
          codigo_servico,nome_servico_referencia,tipo_objeto,tipo_servico,subgrupo,tabela,atualizado_por,atualizado_em
        ) VALUES(?,?,?,?,?,?,?,datetime('now'))
        ON CONFLICT(codigo_servico) DO UPDATE SET
          ...
          atualizado_por=excluded.atualizado_por,
          atualizado_em=datetime('now')
      `).bind(item.codigo,newObj.nomeServico,newObj.tipoObjeto,newObj.tipoServico,newObj.subgrupo,newObj.tabela,user));
    }
    statements.push(auditStmt(env,'servico',item.codigo,'cadastro',JSON.stringify(oldObj),JSON.stringify(newObj),user));
    changed++;
  }
...
async function adminBulkClientAliases(request, env) {
  ...
  source.forEach((raw,index)=>{
    const aliasOriginal=clean(raw?.aliasOriginal),aliasNorm=normText(aliasOriginal),nomeAtual=clean(raw?.nomeAtual),clienteExistente=clean(raw?.clienteExistente),local=normalizeLocal(raw?.localPadrao);
    if(!aliasOriginal&&!nomeAtual&&!clienteExistente&&!clean(raw?.localPadrao))return;
    ...
    if(local===null){errors.push({linha:index+2,alias:aliasOriginal,error:'invalid_local'});return;}
    ...
    items.push({linha:index+2,aliasOriginal,aliasNorm,nomeAtual,clienteExistente,local});
  });
  if(errors.length)return json({ok:false,error:'csv_validation_failed',details:errors.slice(0,50)},400);
  if(!items.length)return json({ok:false,error:'valid_items_required'},400);
  ...
  for(const item of items){
    let target=null;
    const ceNorm=normText(item.clienteExistente);
    const createMode=!ceNorm||ceNorm==='CRIAR NOVO CLIENTE';
    if(!createMode){
      if(/^\d+$/.test(item.clienteExistente)){
        target=byId.get(Number(item.clienteExistente))||null;
      }else{
  ...
  for(let i=0;i<items.length;i+=80){
    const chunk=items.slice(i,i+80),q=await env.DB.prepare(`SELECT alias_normalizado,alias_original,cliente_id FROM atende_cliente_aliases WHERE alias_normalizado IN (${chunk.map(()=>'?').join(',')})`).bind(...chunk.map(x=>x.aliasNorm)).all();
  ...
  for(const [targetKey,spec] of targetSpecs.entries()){
    ...
    const old=byId.get(id)||{},oldName=clean(old.nome_atual),oldLocal=normalizeLocal(old.local_padrao)||'';
  ...
  for(const item of items){
    ...
    statements.push(env.DB.prepare(`INSERT INTO atende_cliente_aliases(cliente_id,alias_original,alias_normalizado,criado_por) VALUES(?,?,?,?) ON CONFLICT(alias_normalizado) DO UPDATE SET cliente_id=excluded.cliente_id,alias_original=excluded.alias_original`).bind(item.targetId,item.aliasOriginal,item.aliasNorm,user));
...
function auditStmt(env,entidade,chave,campo,anterior,novo,usuario){
  return env.DB.prepare(`INSERT INTO atende_admin_historico(entidade,chave,campo,valor_anterior,valor_novo,usuario,criado_em) VALUES(?,?,?,?,?,?,datetime('now'))`)
    .bind(entidade,chave,campo,String(anterior??''),String(novo??''),usuario||'admin');
}
...
const SORT = Object.freeze({
  DATA:'data_postagem','CEP DESTINATARIO':'cep_destinatario','CEP REMETENTE':'cep_remetente',SRO:'codigo_objeto',SERVICO:'codigo_servico',
  'NOME REMETENTE':'nome_remetente','CARTAO POSTAGEM':'cartao_postagem',CONTRATO:'numero_contrato',SISTEMA:'sistema_postagem',
  VALOR:'valor_atendimento',ESTORNO:'estorno',ATENDENTE:'cpf_matricula_atendente','MODALIDADE PAGAMENTO':'modalidade_pagamento','FORMA PAGAMENTO':'forma_pagamento'
});
...
async function legacyAtende(url, env) {
  ...
  const rows=(result.results||[]).map(r=>{r.DATA=formatDateBR(r.DATA);return r}),total=Number(summary?.total||0);
...
async function legacyFilters(env){
  ...
  const body={ok:true};
  specs.forEach(([k],i)=>body[k]=(results[i]?.results||[]).map(r=>clean(r.value)).filter(Boolean));
  return json(body);
}
