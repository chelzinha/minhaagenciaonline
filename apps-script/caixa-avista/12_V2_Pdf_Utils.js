...
function v2GenerateClosingPdf_(env,closureId,context) {
  ...
  try{
    ...
    body.appendTable([['Informação','Valor'],['Saldo inicial',v2Money_(c.opening_cash_cents)],['Receitas em dinheiro',v2Money_(c.cash_revenue_cents)],['Despesas em dinheiro',v2Money_(c.cash_expense_cents)],['Sangrias',v2Money_(c.withdrawals_before_close_cents)],['Saldo esperado',v2Money_(c.expected_cash_cents)],['Saldo contado',v2Money_(c.counted_cash_cents)],['Diferença',v2Money_(c.difference_cents)],['Sangria do fechamento',v2Money_(c.closing_withdrawal_cents)],['Saldo remanescente',v2Money_(c.carryover_cents)]]);
    ...
    if(Number(c.difference_cents)!==0){body.appendParagraph('DIFERENÇA IDENTIFICADA: '+v2Money_(c.difference_cents)).setBold(true);body.appendParagraph('Justificativa: '+String(c.notes||''));}
